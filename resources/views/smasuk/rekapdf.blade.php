...
<head>
    <title>Rekap Surat Masuk / ARSIP SURAT KPU KOTA JAMBI</title>
...
<body>
    ...
    <center> 
        <h4>Daftar Surat Masuk KPU Kota Jambi</h4>
    </center>
    <center>
    ...
    <table class='table table-bordered'>
        ...
        <tbody>
            @forelse($kasbon as $pdf)
                <tr>
                    <td>{{ $pdf->agenda }}</td>
                    <td>{{ $pdf->no }}</td>
                    <td><?= Date('d-m-Y', strtotime($pdf->tgl_surat ?? '')) ?></td>
                    <td><?= Date('d-m-Y', strtotime($pdf->tgl_masuk ?? '')) ?></td>
                    <td>{{ $pdf->dari ?? '-' }}</td>
                    <td>{{ $pdf->hal }}</td>
                </tr>
            @empty
